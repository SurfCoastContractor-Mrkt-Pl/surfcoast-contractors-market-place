      ...
      {showForm && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4 space-y-3">
          ...
          <div>
            ...
            />
            <p className="text-xs text-slate-400 mt-1">Type to search — selecting a result auto-fills the address below.</p>
          </div>
          ...
          <div>
            ...
            <input
              className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              placeholder="e.g. Rain or shine, near the main entrance"
              value={form.notes}
      ...
      {markets.length === 0 && !showForm ? (
        ...
        <div className="space-y-2">
          ...
          {upcomingMarkets.map(m => (
            <MarketCard key={m.id} m={m} onRemove={handleRemove} />
          ))}
          ...
          {pastMarkets.length > 0 && (
            <div className="mt-4">
              ...
              {showPast && pastMarkets.map(m => (
                <MarketCard key={m.id} m={m} onRemove={handleRemove} past />
              ))}
            </div>
          )}
        </div>
...
function MarketCard({ m, onRemove, past }) {
  ...
  return (
    <div className={`flex items-start gap-3 bg-white border rounded-xl p-4 ${past ? 'opacity-60 border-slate-100' : 'border-slate-200'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className="font-semibold text-sm text-slate-800">{m.location || addressLine}</p>
          {m.booth_number && (
        ...
        {m.date && (
          <p className="text-xs text-slate-600 mt-0.5 flex items-center gap-1">
            <CalendarDays className="w-3 h-3" /> {formatDate(m.date)}{timeRange ? ` · ${timeRange}` : ''}
          </p>
        )}
        {addressLine && (
          <p className="text-xs text-slate-400 mt-0.5 truncate">{addressLine}</p>
        )}
        {m.notes && (
          <p className="text-xs text-slate-400 mt-0.5 italic">{m.notes}</p>
        )}
        {mapsUrl && (
