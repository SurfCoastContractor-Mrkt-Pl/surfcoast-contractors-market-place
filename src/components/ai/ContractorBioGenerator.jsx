...
export default function ContractorBioGenerator({ initialData, onGenerated }) {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const response = await base44.functions.invoke('generateContractorBio', initialData);
      setBio(response.data.bio);
      onGenerated?.(response.data.bio);
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-semibold text-slate-900">AI Bio Generator</h3>
      </div>
      ...
      {!bio ? (
        ...
        <div className="space-y-3">
          <div className="p-4 bg-blue-50 rounded-lg">
            <p className="text-slate-900 text-sm leading-relaxed">{bio}</p>
          </div>
