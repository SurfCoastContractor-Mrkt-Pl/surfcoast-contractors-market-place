...
export default function JobCloseout({ scope, role, open, onClose }) {
  ...
  const handleConfirm = async () => {
    ...
    // Check if both sides have now confirmed — fetch fresh scope
    const fresh = await base44.entities.ScopeOfWork.filter({ id: scope.id });
    const updated = fresh?.[0] || { ...scope, ...updateData };

    const bothConfirmed =
      (role === 'contractor' ? true : updated.contractor_closeout_confirmed) &&
      (role === 'customer' ? true : updated.customer_closeout_confirmed);

    const bothSatisfied =
      (role === 'contractor' ? rating : updated.contractor_satisfaction_rating) !== 'unsatisfactory' &&
      (role === 'customer' ? rating : updated.customer_satisfaction_rating) !== 'unsatisfactory';

    if (
      (updated.contractor_closeout_confirmed || (role === 'contractor')) &&
      (updated.customer_closeout_confirmed || (role === 'customer'))
    ) {
      // Both have confirmed — close out
      ...
      // Only award badge progress if both rated satisfactory or better
      ...
      if (satisfactoryRatings.includes(contractorRating) && satisfactoryRatings.includes(customerRating)) {
        // Increment contractor completed_jobs_count
        ...
        // Increment customer completed_jobs_count
