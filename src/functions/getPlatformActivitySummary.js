...
Deno.serve(async (req) => {
  try {
    ...
    const [customers, contractors, payments] = await Promise.all([
    ...
    const completedJobs = await base44.asServiceRole.entities.ScopeOfWork.filter(
      { status: 'closed' },
      '-closed_date',
      1000
    );
    ...
    const newCustomersLast30 = customers.filter(c => 
      c.created_date && new Date(c.created_date) > thirtyDaysAgo
    ).length;

    const newContractorsLast30 = contractors.filter(c => 
      c.created_date && new Date(c.created_date) > thirtyDaysAgo
    ).length;

    const completedJobsLast30 = completedJobs.filter(j => 
      j.closed_date && new Date(j.closed_date) > thirtyDaysAgo
    ).length;
