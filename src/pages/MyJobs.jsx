...
import { Briefcase, Plus, MapPin, DollarSign, Users, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
import { createPageUrl } from '@/utils';
import JobBidsPanel from '@/components/jobs/JobBidsPanel';
...
export default function MyJobs() {
  ...
  const { data: postedJobs, isLoading: jobsLoading } = useQuery({
    ...
    queryFn: () => userEmail ? base44.entities.Job.filter({ poster_email: userEmail }) : Promise.resolve([]),
    enabled: !!userEmail && !isContractor,
  });

  // Fetch all bids for jobs posted by this user
  const { data: allBids } = useQuery({
    queryKey: ['my-job-bids', userEmail],
    queryFn: () => userEmail ? base44.entities.Bid.filter({ customer_email: userEmail }) : Promise.resolve([]),
    enabled: !!userEmail && !isContractor,
  });

  const jobsWithBidCounts = useMemo(() => {
    if (!postedJobs || !allBids) return [];
    return postedJobs.map(job => ({
      ...job,
      bidCount: allBids.filter(b => b.job_id === job.id).length,
      bids: allBids.filter(b => b.job_id === job.id),
    }));
  }, [postedJobs, allBids]);

  const activeJobs = useMemo(() => 
    jobsWithBidCounts.filter(j => j.status === 'open'), 
    [jobsWithBidCounts]
  );
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 text-center">
          ...
          <p className="text-slate-600 mb-6">This section is for customers who post jobs. Contractors can browse and bid on jobs from the Jobs page.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-slate-300">Manage your posted jobs and review contractor bids</p>
            </div>
                  ...
                  <Card key={job.id} className="p-6 hover:shadow-lg transition-shadow">
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {job.trade_needed && (
                          <Badge className="mb-3">{job.trade_needed}</Badge>
                        )}
                      </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-slate-500">{job.bidCount} bids received</span>
                        </div>
                      </div>
                    </div>
                  </Card>
      ...
      {/* Bids Panel */}
      {selectedJob && (
        <JobBidsPanel job={selectedJob} onClose={() => setSelectedJob(null)} />
      )}
    </div>
  );
}