  ...
  try {
    ...
    if (scope.status !== 'closed') {
      return Response.json(
        { error: 'Scope must be closed to create review request' },
        { status: 400 }
      );
    }

    // Check if review request already exists
    const existingRequests = await base44.entities.ReviewEmailRequest.filter({
      scope_id: scope_id,
    });

    if (existingRequests && existingRequests.length > 0) {
      return Response.json(
        { error: 'Review request already exists for this scope' },
        { status: 400 }
      );
    }
    ...
    const reviewRequest = await base44.entities.ReviewEmailRequest.create({
      scope_id: scope_id,
    ...
    console.log(`Review email request created for scope ${scope_id}, scheduled for ${emailSendAt.toISOString()}`);
    ...
    console.error('Review request creation error:', error.message);
